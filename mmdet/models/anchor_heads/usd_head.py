...
class USDHead(nn.Module):
    ...
    def usd_target_single(self, gt_bboxes, gt_labels, gt_coefs, points, regress_ranges, num_points_per_lvl):
        ...
        if self.use_center_sample:
            inside_gt_bbox_mask = self.get_center_sample(gt_bboxes, self.strides, num_points_per_lvl,
                                                         xs.long(), ys.long(), radius=1.5)
        else:
    ...
    def get_center_sample(self, gt, strides, num_points_per, gt_xs, gt_ys, radius=1):
        ...
        for level, n_p in enumerate(num_points_per):
            ...
            stride = int(strides[level] * radius)
