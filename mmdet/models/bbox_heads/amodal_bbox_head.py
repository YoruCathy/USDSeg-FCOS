...
from mmdet.core import (auto_fp16, amodal_bbox_target, delta2bbox, force_fp32,
                        multiclass_nms)
...
class AmodalBBoxHead(nn.Module):
    ...
    def __init__(self,
                 ...
                 method='None',
                 loss_mask=None):
        super(AmodalBBoxHead, self).__init__()
        assert with_cls or with_reg or with_coef
    ...
    def init_weights(self):
        ...
        if self.with_coef:
            nn.init.normal_(self.fc_coef.weight, 0, 0.001)
            nn.init.constant_(self.fc_coef.bias, 0)
    ...
    def forward(self, x):
        ...
        coef_pred = self.fc_coef(x) if self.with_coef else None
        return cls_score, bbox_pred, coef_pred

    def get_target(self, sampling_results, gt_bboxes, gt_coefs, gt_labels,
                   rcnn_train_cfg):
        pos_proposals = [res.pos_bboxes for res in sampling_results]
        neg_proposals = [res.neg_bboxes for res in sampling_results]
        pos_gt_bboxes = [res.pos_gt_bboxes for res in sampling_results]
        pos_gt_labels = [res.pos_gt_labels for res in sampling_results]
        if sampling_results[0].pos_gt_coefs is not None:
